...
async function loadCustomers() {
  ...
  const { data: debts, error: dErr } = await sb
    .from('debts')
    .select('id, customer_id, amount, paid_amount, description, paid, created_at')
    .eq('owner_id', owner)
    .order('created_at', { ascending: false });
...
async function sendSmsReminder(customerId) {
  ...
  const message = `Hello ${c.name}, this is a friendly reminder from your shop that your outstanding balance is ${money(owed)}. Thank you!`;
