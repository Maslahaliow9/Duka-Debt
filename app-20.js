...
function wireChamaBody() {
  ...
  document.querySelectorAll('.chama-tick, .grid-cell').forEach(el => {
    el.onclick = (e) => {
      e.stopPropagation();
  ...
  return `
    <div class="sheet-backdrop" id="export-backdrop">
      <div class="sheet">
        ...
        <button class="primary" id="export-print">🖨️ Print / Save as PDF</button>
...
function printChamaYear(year) {
  ...
  const html = `
    <h1>${shopName ? escapeHtml(shopName) + ' — ' : ''}Mama Chama ${year}</h1>
...
function downloadChamaCSV(year) {
  ...
  a.href = url;
  a.download = `mama-chama-${year}.csv`;
